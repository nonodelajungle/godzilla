    ...
    return (
      <main className="min-h-screen bg-[#f8fbfc] px-4 py-20">
        <div className="mx-auto max-w-3xl rounded-[32px] border border-slate-200 bg-white p-10 text-center shadow-sm">
          <h1 className="text-3xl font-bold tracking-tight text-slate-950">MVP generator not found</h1>
          <p className="mt-4 text-slate-500">Open a saved Buildly project from this browser, then launch the MVP page again.</p>
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-8">
            <CardSection title="Builder prompt" action={<button onClick={() => copyText(pack.buildPrompt)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700">Copy</button>}>
              <pre className="whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-700">{pack.buildPrompt}</pre>
            </CardSection>

            <CardSection title="Execution pack">
              <div className="grid gap-6 lg:grid-cols-2">
                <ListCard title="User stories" items={pack.userStories} />
                <ListCard title="Screens" items={pack.screens} />
                <ListCard title="Data model" items={pack.dataModel} />
                <ListCard title="Recommended stack" items={pack.stack} />
              </div>
            </CardSection>

            <CardSection title="14-day roadmap">
              <div className="space-y-3">
                {pack.roadmap14Days.map((item) => <div key={item} className="rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-700">{item}</div>)}
              </div>
            </CardSection>
          ...
          <div className="space-y-8">
            <CardSection title="Core product definition">
              <div className="space-y-4">
                <InfoCard title="Target user" body={pack.targetUser} />
                <InfoCard title="Core outcome" body={pack.coreOutcome} />
                <ListCard title="Integrations" items={pack.integrations} />
              </div>
            </CardSection>

            <CardSection title="Build now">
              <ListCard title="Must-have features" items={brief.features} />
            </CardSection>

            <CardSection title="Do not build yet">
              <ListCard title="Keep out of v1" items={brief.doNotBuild} />
            </CardSection>

            <CardSection title="Investor memo">
              <div className="text-lg font-semibold tracking-tight text-slate-950">{investorMemo.title}</div>
            ...
            <CardSection title="Recent leads">
              <div className="space-y-3">
                {leads.length === 0 ? (
...
function CardSection({ title, children, action }: { title: string; children: React.ReactNode; action?: React.ReactNode }) { return <section className="rounded-[28px] border border-slate-200 bg-white p-7 shadow-sm"><div className="mb-4 flex items-center justify-between gap-3"><div className="text-sm font-semibold text-slate-900">{title}</div>{action}</div>{children}</section>; }
